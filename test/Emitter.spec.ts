    ...
    [emitter, extended].forEach(em => {
      ...
      describe(m('special cases tests'), () => {
        ...
        it('should throw if called from wrong context', () => {
          let cb
          expect(() => { (cb = em.on)       ("ev", ()=>{}) } ).to.throw()
          expect(() => { (cb = em.once)     ("ev", ()=>{}) } ).to.throw()
          expect(() => { (cb = em.emit)     ("ev", 1, 2, 3) } ).to.throw()
          expect(() => { (cb = em.off)      ("ev", ()=>{}) } ).to.throw()
          expect(() => { (cb = em.triggers) ("ev", ()=>{}) } ).to.throw()
        })
